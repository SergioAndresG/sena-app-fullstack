  ...
  try {
    ...
    if (respuesta.data.archivo_existente) {
      Swal.fire({
        ...
        text: "Ya existe un archivo individual generado para este aprendiz",
        showDenyButton: true,
...
const consultarFicha = async () => {
  if (ficha.value.trim() !== '' && documento_ap.value.trim() !== '') {
    cargarAprendicesFicha(ficha.value, documento_ap.value)
  } else {
    Swal.fire({
      icon: "error",
      title: "Oops...",
...
function actualizarAprendiz(datosEditados) {
  ...
  if (index !== -1) {
    ...
    const datosParaExportar = {
      ...
      departamento: datosEditados.departamento || '',
